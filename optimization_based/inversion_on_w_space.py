...
parser.add_argument('--image_path', type=str, default='data/36_front.png')
...
save_image(gt_image, os.path.join(opt.save_path, "gt.jpg"), normalize=True)
...
for i in tqdm(range(n_iterations)):
    ...
    # frame, _ = generator.forward_with_frequencies(w_frequencies + w_frequency_offsets, w_phase_shifts + w_phase_shift_offsets, **options)
    loss = torch.nn.MSELoss()(frame, gt_image)

    # print(frame.shape,gt_image.shape)
    # p_loss = percept(frame, torch.unsqueeze(gt_image, 0)).sum()
    # loss += p_loss
    loss = loss.mean()

    writer.add_scalar('image_loss', loss, i)
    ...
    if i % 100 == 0:
        ...
        writer.add_images('image_z', frame, i)
        # with torch.no_grad():
...
save_image(frame, os.path.join(opt.save_path, "z.jpg"), normalize=True)
meta = {
